...
class DataIngestion:
    ...
    def split_data_into_train_test(self, dataframe: pd.DataFrame):
        try:
            ...
            logging.info('Exiting split_data_into_train_test of DataIngestion class')

            dir_path = os.path.dirname(self.data_ingestion_config.training_file_path)
            os.makedirs(dir_path, exist_ok=True)
            logging.info('Created directory for training data')
            
            train_set.to_csv(self.data_ingestion_config.training_file_path, index=False, header=True)
            test_set.to_csv(self.data_ingestion_config.testing_file_path, index=False, header=True)
            logging.info('Data saved into training and testing files')
